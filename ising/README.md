...
Writing this out, $\log p(D\mid\psi) = \sum_{c\in\{m,m^2,m^4\}} \Biggl[ -\tfrac12\, {\Phi^{(c)}}^\top \bigl(K^{(c)}\bigr)^{-1} \Phi^{(c)} -\tfrac12\log\det K^{(c)} -\tfrac{n}{2}\log(2\pi) \Biggr]$, where $K^{(c)}_{ij} = k_{\ell_f,\eta_f}(z_i,z_j) + \sigma_i^2 \delta_{ij}$.
